...
public sealed class CommandRegistrationGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor FoundCommandDescriptor = new(
        id: "TCG001",
        title: "Command registered",
        messageFormat: "Auto-registering IMenuCommand: {0}",
        category: "CommandRegistration",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true);

    private static readonly DiagnosticDescriptor NoCommandsDescriptor = new(
        id: "TCG002",
        title: "No commands found",
        messageFormat: "No IMenuCommand implementations found in compilation",
        category: "CommandRegistration",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true);
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> types)
    {
        if (types.IsEmpty)
        {
            context.ReportDiagnostic(Diagnostic.Create(NoCommandsDescriptor, Location.None));
            return;
        }

        foreach (var type in types)
            context.ReportDiagnostic(Diagnostic.Create(FoundCommandDescriptor, Location.None, type.ToDisplayString()));
